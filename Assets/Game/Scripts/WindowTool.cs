...
public class WindowTool : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			SetResolution(1600, 900, WindowType.ResizableWindow);
		}
		//设置为可调整大小的窗口模式
		if (Input.GetKeyUp(KeyCode.F3))
		{
			SetResolution(1366, 768, WindowType.FixedSizeWindow);
		}
		//设置全屏
		if (Input.GetKeyUp(KeyCode.F4))
		{
			Resolution[] resolutions = Screen.resolutions; //获取设置当前屏幕分辩率
			var index = resolutions.Length - 1;
			Screen.SetResolution(resolutions[index].width, resolutions[index].height, true); //设置当前分辨率
			Screen.fullScreen = true; //设置成全屏,
		}
	}

	public void SetResolution(int width, int height, WindowType windowType, ZOrder zOrder = ZOrder.NoTopMost)
	{
		switch (windowType)
		{
	...
	private IEnumerator SetResizableWindow(int width, int height, ZOrder zOrder = ZOrder.NoTopMost)
	{
		...
		SetWindowLong(hWndSelf, GWL_STYLE,
			(int) (GetWindowLong(hWndSelf, GWL_STYLE) | WindowStyle.WS_SIZEBOX | WindowStyle.WS_MAXIMIZEBOX));
		SetZOrder(zOrder);
	}
	...
	{
		SetWindowPos(hWndSelf, new IntPtr((int) zOrder), 0, 0, width, height,
	...
	{
		SetWindowPos(hWndSelf, IntPtr.Zero, 0, 0, width, height,
			SWP_NOMOVE | SWP_NOZORDER | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
	}

	public void SetZOrder(ZOrder zOrder)
	{
		SetWindowPos(hWndSelf, new IntPtr((int) zOrder), 0, 0, 0, 0,
