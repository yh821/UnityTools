...
public class TexturePackerEditor : EditorWindow
{
    ...
    static void BuildAtlas(string particlePath, string atlasPath, ETrimMode mode = ETrimMode.CropKeepPos, int border = 1, int shape = 1, int maxSize = 2048)
    {
        System.Diagnostics.Process process = new System.Diagnostics.Process();
        process.StartInfo.FileName = Application.dataPath + "/Editor/TexturePacker/TPackageTool.bat";
        process.StartInfo.UseShellExecute = true;
        process.StartInfo.Arguments = string.Format("\"{0}\" \"{1}\" {2} {3} {4} {5} {6}", particlePath, TexturePackerPath, atlasPath, mode, maxSize, border, shape);
        process.Start();
        process.WaitForExit();
        process.Close();
    }
    ...
    private void OnGUI()
    {
        ...
        inputTexture = (Texture2D)EditorGUILayout.ObjectField("", inputTexture, typeof(Texture2D), false, GUILayout.MaxWidth(64));
        EditorGUILayout.BeginVertical();
        ...
        if (inputTexture != null && Directory.Exists(particlePath))
        {
            ...
            {
                string altasPath = Application.dataPath.Replace("Assets", "") + assetPath.Substring(0, assetPath.LastIndexOf(".png"));
